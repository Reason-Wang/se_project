...
class orderScreen(QWidget, Ui_Form):
    ...
    def refresh_order_table(self):     #刷新订单表
        ...
        for k in range(0,(query_row_number()[0][0])):
            ...
            self.tableWidget.setCellWidget(k,9,bt)
        #下面的函数就是商品ID变成商品名字然后写进表里的函数
        for t in range(0,(query_row_number()[0][0])):       #先正常建表,然后单独该字典
            mer_dict = ast.literal_eval((query_bincheng_order_table_mer_dict_according_orderID(self.tableWidget.item(t,0).text())[0][0]))
            string=''
            for i, j in mer_dict.items():
                string = string+(str(query_name_accordingID(i)[0][0])+':'+str(j)+',')
            string=string.strip(',')
